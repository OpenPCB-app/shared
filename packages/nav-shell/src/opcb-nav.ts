...
type Mode = "marketing" | "community" | "workspace";
...
const NAV_LINKS: ReadonlyArray<{
  key: string;
  label: string;
  /** "marketing" → resolved against marketing-base; "app" → against app-base. */
  area: "marketing" | "app";
  path: string;
}> = [
...
export class OpcbNav extends HTMLElement {
  ...
  private readonly onFocus = (): void => this.syncDynamic();
  ...
  private get marketingBase(): string {
    return (
      this.getAttribute("marketing-base") ?? DEFAULT_MARKETING_BASE
    ).replace(/\/$/, "");
  }
  ...
  private url(area: "marketing" | "app", path: string): string {
...
<header class="nav">
  ...
  private ctaHtml(session: SessionHint | null): string {
    if (session) {
   ...
   * thing dropped.
   */
  private menuCtaHtml(session: SessionHint | null): string {
  ...
  private toggleTheme(): void {
    ...
    if (btn) btn.innerHTML = next === "light" ? SUN_SVG : MOON_SVG;
    this.dispatchEvent(
      new CustomEvent("opcb-nav:theme", {
        detail: { theme: next },
        bubbles: true,
        composed: true,
      }),
  ...
  private syncDynamic(): void {
    ...
    applyTheme(getTheme());
    const btn = this.root.querySelector("[data-theme-toggle]");
    if (btn) btn.innerHTML = getTheme() === "light" ? SUN_SVG : MOON_SVG;
    const session = getSession();
    const key = session ? session.initials + "|" + session.name : "";
    if (key !== this.lastSessionKey) this.render();
  }
}
